...
export const generateTipQueue = (suspect, weapon, place) => {
  ...
  let stage1 = [suspectTips.pop(), weaponTips.pop()]
  stage1.sort(() => Math.random() - 0.5)

  let stage2 = [weaponTips.pop(), placeTips.pop()]
  stage2.sort(() => Math.random() - 0.5)

  let stage3 = [...suspectTips, ...weaponTips, ...placeTips]
  stage3.sort(() => Math.random() - 0.5)

  let result = [...stage1, ...stage2, ...stage3]
  uselessTips.map((el) => {
    result.splice(Math.floor(Math.random() * 10), 0, el)
  })
